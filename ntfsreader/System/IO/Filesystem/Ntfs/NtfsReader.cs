...
namespace System.IO.Filesystem.Ntfs
{
    public sealed partial class NtfsReader : IDisposable
    {
        ...
        private sealed class NodeWrapper : INode
        {
            ...
            public static DateTime ReturnLastWriteDate(string filepath)
            {
                ...
                if (hFile != IntPtr.Zero)
                {
                    try
                    {
                        long creationTime, lastAccessTime, lastWriteTime;

                        // Attempt to retrieve the file times (we are only interested in the last write time)
                        if (GetFileTime(hFile, out creationTime, out lastAccessTime, out lastWriteTime))
                        {
                            // Convert last write time to DateTime
                            DateTime lastWriteDateTime = DateTime.FromFileTime(lastWriteTime);
                            return lastWriteDateTime;
                        }
                    }
                    finally
                    {
                        // Ensure the file handle is always closed, even if an error occurred
                        CloseHandle(hFile);
                    }
                }

                // Return null if there was an error or the file doesn't exist
                return DateTime.Now;
            }
            public DateTime LastChangeTime
            {
                get 
                {
                    ...
                    var lastAccessTimeRaw = _reader._standardInformations[_nodeIndex].LastAccessTime;

            
                   
                   
                        long lastAccessTimeAsLong = lastAccessTimeRaw is ulong ulongTime ? (long)ulongTime : (long)lastAccessTimeRaw;

                    return DateTime.FromFileTime(lastAccessTimeAsLong);
            ...
            public DateTime LastAccessTime
            {
                get 
                {
                    if (_reader._standardInformations == null)
                        throw new NotSupportedException("The StandardInformation haven't been retrieved. Make sure to use the proper RetrieveMode.");
                    var lastAccessTimeRaw = _reader._standardInformations[_nodeIndex].LastChangeTime;

                    // Make sure LastAccessTime is a valid 64-bit integer

                    // If it's a ulong, cast it to long first
                    long lastAccessTimeAsLong = lastAccessTimeRaw is ulong ulongTime ? (long)ulongTime : (long)lastAccessTimeRaw;
        ...
        private unsafe Node[] ProcessMft1()
        {
            uint bufferSize = (Environment.OSVersion.Version.Major >= 6 ? 256u : 64u) * 8192;
            byte[] data = ArrayPool<byte>.Shared.Rent((int)bufferSize); // Reuse buffer to avoid GC pressure

            try
            {
                fixed (byte* buffer = data)
                {
                    // Read the $MFT record from disk into memory.
                    ReadFile(buffer, _diskInfo.BytesPerMftRecord, _diskInfo.MftStartLcn * _diskInfo.BytesPerSector * _diskInfo.SectorsPerCluster);

                    // Fixup the raw data from disk
                    ...
                    for (uint nodeIndex = 1; nodeIndex < maxInode; nodeIndex++)
                    {
                        ...
                        {
                            if (!ReadNextChunk(buffer, bufferSize, nodeIndex, fragmentIndex, dataStream, ref BlockStart, ref BlockEnd, ref Vcn, ref RealVcn))
                                break;

                            totalBytesRead += (BlockEnd - BlockStart) * _diskInfo.BytesPerMftRecord;
                        }

                        FixupRawMftdata(buffer + (nodeIndex - BlockStart) * _diskInfo.BytesPerMftRecord, _diskInfo.BytesPerMftRecord);
                        ...
                        Node newNode;
                        if (!ProcessMftRecord(buffer + (nodeIndex - BlockStart) * _diskInfo.BytesPerMftRecord, _diskInfo.BytesPerMftRecord, nodeIndex, out newNode, streams, false))
                            continue;
            ...
            {
                ArrayPool<byte>.Shared.Return(data); // Ensure buffer is returned to the pool
            }
        }
