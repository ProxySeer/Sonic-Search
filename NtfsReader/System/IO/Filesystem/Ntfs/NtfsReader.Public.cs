...
namespace System.IO.Filesystem.Ntfs
{
    ...
    public partial class NtfsReader
    {
        ...
        public List<INode> GetNodes(string rootPath)
        {
            ...
            var nodeBag = new ConcurrentBag<INode>();
            int nodeCount = _nodes.Length;

            var results = new List<INode>[Environment.ProcessorCount];

            Parallel.For(0, results.Length, coreId =>
            {
                var localList = new List<INode>();
                for (int i = coreId; i < _nodes.Length; i += results.Length)
                {
                    var node = _nodes[i];
                    if (node.NameIndex != 0)
                    {
                        var fullName = GetNodeFullNameCore((uint)i);
                        if (fullName.StartsWith(rootPath, StringComparison.InvariantCultureIgnoreCase))
                        {
                            localList.Add(new NodeWrapper(this, (uint)i, node));
                        }
                    }
                }
                results[coreId] = localList;
            });

            var nodes = results.SelectMany(x => x).ToList();
